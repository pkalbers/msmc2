...
-p, --targetPosition <int>: return posteriors only for target position");
}
...
void decodeWithHmm(PSMC_hmm hmm) {
  ...
  double[][] posteriors;
  double [] positions; // MOD

  // MOD
  double upper = to!double(target) + (to!double(stride) / 2.0);
  double lower = to!double(target) - (to!double(stride) / 2.0);
  ...
  // MOD
  auto timeBoundaries = hmm.propagationCore.getPSMC().timeIntervals.boundaries.dup;
